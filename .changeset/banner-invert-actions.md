...
The two properties carry opposite obligations, and each flavour's fallback is what decides it. `--current-accent` falls back to `currentcolor`, which a disabled host has already muted, so `invert` gates its own fade and the offering container owns every state — `Banner` pairs each accent with a `.4` counterpart. `--current-label` falls back to the absolute `#surface`, which nothing above touches, so `primary` always fades and a container offers the live color only.
