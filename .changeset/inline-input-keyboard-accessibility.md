...
- A keyboard focus ring (`#focus` token, rounded) appears on the display element when it receives keyboard focus (`useFocusRing`'s `isFocusVisible`). It is suppressed automatically while editing (focus is on the inner input) and when `keyboardActivation={false}` (so the host's focus ring is the one users see — as in `Tabs`). New `focused` modifier exposed on the root for consumers.
