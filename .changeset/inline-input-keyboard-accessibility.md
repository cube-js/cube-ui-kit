...
**Tabs**: editable tabs continue to be driven by `F2` / context-menu rename at the tab-button level; `InlineInput` now receives `keyboardActivation={false}` from `TabButton` so the inline title does not introduce an extra tab stop inside the tab.
