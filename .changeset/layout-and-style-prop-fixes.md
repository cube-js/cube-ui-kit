...
`Item`'s `prefix` slot no longer sits flush against the label. The grid contributes no gap and `Label` drops its own leading padding for any start content, including a prefix, so nothing was left between the two — while the same glyph in the `icon` slot looked spaced, because that slot is a `$size`-wide square that centres a smaller glyph.
