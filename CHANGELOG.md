...
- [#1425](https://github.com/cube-js/cube-ui-kit/pull/1425) [`86ea64d7`](https://github.com/cube-js/cube-ui-kit/commit/86ea64d7a98f836dd6a57076e505312087d6393a) Thanks [@tenphi](https://github.com/tenphi)! - Add `tooltip` to `Checkbox`: a tooltip shown on hover over the box and its inline label, standalone or inside a `Checkbox.Group`, and also while the checkbox is disabled — a place to say why an option is unavailable. It opens on keyboard focus too, and describes the input to screen readers. It takes a string or `TooltipProvider` props. It is separate from `labelTooltip`, the info badge next to a field label.

- [#1424](https://github.com/cube-js/cube-ui-kit/pull/1424) [`f0824b25`](https://github.com/cube-js/cube-ui-kit/commit/f0824b25c01c25148a4ca62a99baba2f99b58d73) Thanks [@tenphi](https://github.com/tenphi)! - `Dialog`'s `Footer` now draws a top border while the dialog's `Content` scrolls, and none while it fits. The footer carries a `content-overflow` modifier for as long as the body overflows, tracked through resizes and content changes, so the line separates the actions from a body that continues beneath them without cluttering a short dialog. Set `border` in the footer's own styles to keep the line always or never draw it. `Content` now renders `data-id="Content"`, which is how the dialog finds its body.
...
- [#1422](https://github.com/cube-js/cube-ui-kit/pull/1422) [`565dc93d`](https://github.com/cube-js/cube-ui-kit/commit/565dc93d8b71292218cebb2a96c70de4d7193518) Thanks [@tenphi](https://github.com/tenphi)! - Let popover fields anchor to an element other than their own trigger. `Select`, `Picker`, `FilterPicker`, `ColorPicker`, `ColorInput`, `DatePicker`, `DateRangePicker`, `DateRangeSeparatedPicker` and `PeriodPicker` now accept `targetRef`, matching the prop `DialogTrigger` already had, so adjacent controls of different widths can line their popovers up with one shared container. The same components (except `FilterPicker`, which already had it) also accept `placement`, since anchoring to a wider container is only useful alongside control over the alignment. Both default to the current behaviour, so existing callers are unaffected.

- [#1421](https://github.com/cube-js/cube-ui-kit/pull/1421) [`a8295d41`](https://github.com/cube-js/cube-ui-kit/commit/a8295d413cb6a25820937cb94af541e7b14c3079) Thanks [@tenphi](https://github.com/tenphi)! - Add a soft brand-purple `Tag` theme: `<Tag theme="primary">` renders the pale card — tinted border, strong colored text — at the primary hue, alongside the existing `default`, `danger`, `success`, `warning` and `note` cards. Previously the brand hue was reachable only through `theme="special"`, which is a solid fill, so designs calling for a soft purple chip had to borrow `note` one hue over. `special` is unchanged and still renders the solid fill.

### Patch Changes

- [#1426](https://github.com/cube-js/cube-ui-kit/pull/1426) [`17416c42`](https://github.com/cube-js/cube-ui-kit/commit/17416c422d8c2f878be168938c132d88fb7dcd94) Thanks [@tenphi](https://github.com/tenphi)! - Fix `Menu` items running their action twice from the keyboard: Enter or Space on a focused item called the menu's `onAction`, and the item's own `onAction`, twice — in a standalone `Menu`, a `Menu.Section`, a `Menu.Trigger` popover and a submenu alike (clicks were unaffected). Enter on a `Menu.SubMenuTrigger` now only opens the submenu, instead of also calling the parent menu's `onAction` with the trigger's key, and a click on a submenu trigger after opening it from the keyboard opens it straight away rather than after the hover delay.

- [#1425](https://github.com/cube-js/cube-ui-kit/pull/1425) [`86ea64d7`](https://github.com/cube-js/cube-ui-kit/commit/86ea64d7a98f836dd6a57076e505312087d6393a) Thanks [@tenphi](https://github.com/tenphi)! - A `Radio`'s `tooltip` now opens when the option takes keyboard focus, not only on hover, and the focused input is described by it, so screen readers announce it. It follows arrow-key navigation between options, and any `aria-describedby` the radio already had is kept alongside it.
...
- [#1416](https://github.com/cube-js/cube-ui-kit/pull/1416) [`cf3e03b9`](https://github.com/cube-js/cube-ui-kit/commit/cf3e03b987885f5edbd15fdbef2a13de1f7b7330) Thanks [@tenphi](https://github.com/tenphi)! - Four places where a style or a size did not do what it said.
...
- [#1374](https://github.com/cube-js/cube-ui-kit/pull/1374) [`9adb076f`](https://github.com/cube-js/cube-ui-kit/commit/9adb076f00a75357b4e25c8e09e122d94eecd621) Thanks [@tenphi](https://github.com/tenphi)! - Update Tasty to 3.5.0 (from 3.3.1). No changes were needed in the kit; two things matter to apps built on it.
