...
| --- | --- | --- |
| Legacy bound | `name` is set, not inside the deprecated `<Field>`, not `unsafe__isDisabled` | The field's value/onChange/onBlur and validation state merged over the props, with an incremental id derived from the form name and the field name (`email`, `email_1`, `settings_email`, …) |
| Modern bound | A modern controller plus `name`, not inside `<Field>` or disabled | Per-field value/error/status subscription, commit-time registration, and a name-prefixed React `useId()` id; no creator or unrelated-field subscription |
...
Form instances are branded (see `Form/backend.ts`). `Form.useController()` creates the modern controller; named inputs and custom controls using `useFieldProps` bind through `use-field-binding.ts`. The legacy adapter runs inert on this path. Modern registration and option updates happen in layout effects, never during render. A changed name/controller releases the old registration immediately; values remain retained by default. `preserve={false}` removes an inactive value after one microtask so Strict Mode effect replay can reconnect safely; reset can restore its baseline. Controller values/defaults, including explicit `undefined` and `null`, precede field defaults. Every duplicate input has its own stable React id and registration token. Only value, errors, or status changes rerender the input; metadata belongs in `Form.useSelector` or `Form.Subscribe`.

`Form.Item` remains legacy-only: modern custom controls use `useFieldProps` and `wrapWithField` directly. Do not call a controller getter during render to implement a reactive input. Modern roots expose their controller through `useFormProps` so named inputs cannot silently bind to an outer legacy form. Standalone inputs and explicit `form={undefined}` remain detached. Legacy roots and `FormScopeMask` mask modern context. The shared hook strips the controller from resolved input props before passing them to React Aria, whose `form` attribute is a DOM id. Modern rules run after field events or explicit controller validation, with AbortSignal cancellation and `errorPolicy` ordering. Forward `deps` for external validator inputs and `dependsOn` for declared field dependencies; `rulesKey` opts into manual function versioning and skips source comparison; declarative constraints are always compared. `deps` alone does not disable source comparison. Validator reads cancel in-flight work but only declared `dependsOn` paths schedule dependent revalidation. Equivalent inline rules must not restart pending work. Root and action helpers use selector subscriptions; never read mutable legacy flags from a modern controller.
...
`field={form.field(path, options)}` carries controller/path/options into the shared hook, including nested tuple paths. Construction is pure; registration stays in the commit effect. `useFieldProps` resolves descriptor identity/options and strips `field`, `deps`, and `dependsOn` before DOM forwarding. Keep the base value API correct (booleans for toggles, numbers for numeric controls, selection keys for selectors). The descriptor generic describes accepted model values, not only emitted changes: include `null | undefined` when the binding handles missing values. Normalize only for display; never write the fallback into the form on mount. Add nullable-model consumer fixtures and preserve nulls through submission/reset in runtime tests. Do not add another registration path or read controller getters during render. Existing `name` bindings and legacy rule props keep their compatibility contract.

## Tuple field names

Modern inputs accept `name={readonlyTuple}` without a descriptor. The shared binding retains the path for store access and converts it to an escaped string for DOM names/ids; do not coerce arrays with `String()` or `.join()` in a component. `useFieldProps` returns a string `name`, including standalone/masked inputs, so custom controls can safely forward resolved props. Native React Aria name types must not narrow public field names back to `string`: omit `name` alongside `form`, or explicitly redeclare the kit's name where the Aria interface is extended directly. Legacy bindings accept string names only.
