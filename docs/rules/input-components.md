...
`Form.Item` remains legacy-only: modern custom controls use `useFieldProps` and `wrapWithField` directly. Do not call a controller getter during render to implement a reactive input. Modern roots expose their controller through `useFormProps` so named inputs cannot silently bind to an outer legacy form. Standalone inputs and explicit `form={undefined}` remain detached. Legacy roots and `FormScopeMask` mask modern context. Strip the controller from resolved input props before passing them to React Aria, whose `form` attribute is a DOM id.
