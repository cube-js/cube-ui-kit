...
|----------|------|-------------|
| `type` | `'default' \| 'narrow' \| 'file' \| 'panel' \| 'radio'` | Current tab type |
...
<Story of={TabsStories.PanelType} />

### Sizes

- `xsmall` - Extra small size with t4 typography (radio type only)
- `small` - Small size with t4 typography
...
Enable delete buttons with the `onDelete` callback. When no `menu` is defined, a delete button appears on each tab.
