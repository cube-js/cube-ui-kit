...
When `isReorderable` and `showTabPicker` are both enabled, the tab picker dropdown also supports drag-and-drop reordering. Items in the dropdown display a grip handle and can be dragged to reorder. The same `onReorder` callback is shared between the horizontal tab bar and the vertical tab picker.
