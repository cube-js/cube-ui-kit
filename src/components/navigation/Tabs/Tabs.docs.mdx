...
- **`showTabPicker`** `boolean | 'auto'` (default: `false`) — Show dropdown tab picker. Each entry carries its tab's `icon` and `tooltip`
...
**Note:** Radio type supports `large` (default, 40px total) and `medium` (32px total) sizes, mapped to Item button sizes `medium` (32px) and `xsmall` (24px) respectively. A radio tab given less room than its title (an explicit `width`, say) truncates the title and shows it in a tooltip.
...
<Story of={TabsStories.PlacementBottom} />

<Story of={TabsStories.PlacementLeft} />

<Story of={TabsStories.PlacementRight} />
...
<Story of={TabsStories.VerticalFile} />

<Story of={TabsStories.VerticalRadio} />
...
Action for a single tab's own `actions` slot, where it renders to the left of the built-in menu trigger and delete button. It is an [ItemAction](/docs/actions-itemaction--docs), the same component those built-ins use, so it picks up the tab's size and theme without being configured. Pressing one runs its handler without selecting the tab.

Not to be confused with `Tabs.Action`, which is sized for the tab bar's `prefix` / `suffix` slots rather than for a tab.

<Story of={TabsStories.WithTabActions} />

```jsx
<Tab
  key="tab1"
  title="Tab 1"
  actions={<Tab.Action icon={<PinIcon />} aria-label="Pin tab" onPress={handlePin} />}
>
  Content
</Tab>
```
...
- **`actions`** `ReactNode` — Custom actions to render in the tab, left of the built-in menu trigger and delete button. Use `Tab.Action`
...
Combine editable tabs with a menu for rename, duplicate, and delete actions. The inline editing UI is provided by [InlineInput](/docs/content-inlineinput--docs), which can also be used standalone elsewhere.

> For editable tabs with a string title the `tooltip` prop is rendered by `InlineInput` rather than by the surrounding tab button. This keeps the tooltip in sync with the editing UI (and avoids a double tooltip) — the trigger area is the title content, not the entire tab button. Long titles also get an automatic ellipsis + overflow tooltip without any extra configuration. An editable tab whose `title` is a ReactNode shows its `tooltip` on the tab button instead, hidden while the title is being edited.
