...
<Meta title="Overlays/TooltipProvider" />
...
- **`qa`** `string` — `data-qa` for the rendered tooltip, so a test can find this trigger's tooltip rather than whichever one is open. Components that take a `tooltip` object accept it there too: `tooltip={{ title: 'Copy', qa: 'CopyTooltip' }}`
