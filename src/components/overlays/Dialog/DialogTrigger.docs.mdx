...
- **`shouldCloseOnInteractOutside`** `(element: Element) => boolean` — Function to determine if a popover type Dialog should close on outside interaction. Asked before the built-in dismiss behaviours, so returning `false` keeps the popover open even for a `Button` / `ItemButton` that would otherwise dismiss it — and the pressed control still receives its own click. The element passed is the deepest node under the pointer (for a `Button` that is the label inside it, not the `<button>`), so guard with `contains()` rather than an identity check
