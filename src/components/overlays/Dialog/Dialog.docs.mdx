...
3. **Wrapping the slots in a form**: A `<form>` may sit directly between `Dialog` and its slots, so the submit button is inside it. Dialog hands that form its own flex context, so `Content` still scrolls and `Footer` still pins with no styles of your own:
   ...
   Only a **direct child** form is treated this way; a form nested deeper is your own layout and is left alone.
