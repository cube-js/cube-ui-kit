...
export const WithSubMenus: StoryFn<CubeCommandMenuProps<any>> = (args) => {
  const handleAction = (key: string) => {
    console.log('Action selected:', key);
  };
  ...
  return (
    <CommandMenu {...args} onAction={handleAction}>
      <CommandMenu.Item key="copy" icon={<IconCopy />} hotkeys="Ctrl+C">
        Copy
      </CommandMenu.Item>

      <CommandMenu.SubMenuTrigger>
        <CommandMenu.Item key="share" icon={<IconSelect />}>
          Share
        </CommandMenu.Item>
      ...
      <CommandMenu.Item key="cut" icon={<IconCut />} hotkeys="Ctrl+X">
        Cut
      </CommandMenu.Item>

      <CommandMenu.SubMenuTrigger>
        <CommandMenu.Item key="format">Format</CommandMenu.Item>
        <Menu onAction={handleAction}>
      ...
      <CommandMenu.Item key="paste" icon={<IconClipboard />} hotkeys="Ctrl+V">
        Paste
      </CommandMenu.Item>
    </CommandMenu>
  );
...
WithSubMenus.args = {
  searchPlaceholder: 'Search commands with submenus...',
  autoFocus: true,
  width: '20x 50x',
};
...
WithSubMenus.parameters = {
  docs: {
    description: {
      story:
        'CommandMenu supports submenus using SubMenuTrigger. Submenus can be nested to multiple levels and work with search filtering. The entire menu hierarchy closes when an action is selected.',
    },
  },
};
