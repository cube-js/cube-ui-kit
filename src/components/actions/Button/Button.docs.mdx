...
- **`size`** `'xsmall' | 'small' | 'medium' | 'large' | 'xlarge'` (default: `medium`) — Button size
...
- **`children`** `string` — Button label or custom content
- **`isLoading`** `boolean` (default: `false`) — Show loading spinner and disable interactions (default: false)
- **`isSelected`** `boolean` (default: `false`) — Marks the button as pressed / selected (toggle) (default: false)
