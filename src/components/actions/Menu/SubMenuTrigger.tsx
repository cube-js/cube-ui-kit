...
function InternalSubMenuTrigger(props: InternalSubMenuTriggerProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    // Enter or Space opens submenu. Preventing the default keeps `MenuItem`
    // from passing the key on to React Aria's item handler, which would click
    // the item and run its action as well.
  ...
  const triggerContextValue = useMemo(
    () => ({
      ...
      onAction: isDisabled
        ? undefined
        : () => {
            // A pointer press: Enter and Space open the submenu in
            // `handleKeyDown` and never reach the item's action.
            state.open();
          },
    }),
    [
