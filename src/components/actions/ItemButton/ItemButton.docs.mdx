...
- **The wrapper element is kept once a row has had actions**, even if they later go away. Adding or removing a DOM level around the button would remount it — dropping focus, any running transition, and the node every ref is holding — so a row whose actions depend on a permission or a loading flag stays stable. An empty run reserves no space.
