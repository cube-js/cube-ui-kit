...
- **`resizeGripPlacement`** `'inside' | 'corner' | 'outside'` — Where a widget's resize grips sit. Unset, **it resolves itself from the widget's content**: a widget holding a nested `Board` gets `'outside'`, and every other widget gets `'inside'`. Set it to override that, per board or per widget.

  - `'inside'` tucks each grip into the widget box. The hit-zone is a band along the edge, inside the widget, so it sits over the widget's own content — which is fine for a leaf widget and is why a container resolves the other way.
  - `'outside'` puts every grip in the grid gutter beyond the widget's edge, as a padded control of its own: a pill along each edge, and the same corner angle the other two placements draw, stepped off the corner into the square where two gutters cross. The control **is** the hit-zone — one element, so what you see is exactly what you can grab — and it never covers the widget's own content or a neighbour's.
  - `'corner'` centres a corner grip on the widget's corner, straddling the edge, so it lines up with a control centred on the opposite corner (`cornerChrome`). Its hit-zone reaches half a grip inward, matching the half-dot painted there, and outward as far as half the gutter allows. Edge axes stay `'inside'`.

  Whichever placement applies, grips are revealed on hover, focus or resize, and **only one widget's at a time**: the pointer is inside every ancestor at once, so a container stands down while the pointer is on one of its children. To resize a child you go inside it; to resize its container you go to the container's outer edge.

  An `'outside'` grip needs gutter to sit in — at least 8px of `margin` on each axis. Below that it keeps its size and overhangs the neighbouring widgets, and the board warns in development; raise `margin`, or set `resizeGripPlacement` explicitly to put the grips back inside. The corner angle is the one exception: it shrinks to whatever the gutter leaves it rather than reaching past it onto the diagonal neighbour, and reaches its full size at 12px of `margin`.

  Why the resolution exists: an inner board with no `containerPadding` — which `isAligned` sets — puts its last child's corner on exactly the same point as its host widget's. With both widgets drawing grips in the same place, the container's hit-zone covered the child's and the bottom-right child could not be resized at all. Sending the container's affordance outward and the child's inward leaves the two nothing in common, so nothing has to be arbitrated. Where they do still overlap — an explicit `'corner'` or `'inside'` on a container, or a gutter too thin — a press goes to the **innermost** handle under the pointer, and a container thereby left with no way to resize itself warns in development.
...
- **`isAligned`** `boolean` (default: `false`) — Align a nested board with its ancestor `Board`'s layout. Only takes effect when the board is nested inside another `Board`'s widget. When set, every cell matches the parent's cell size exactly, which means it takes over three of the board's own metrics: `rowHeight` and `margin` are ignored in favour of the parent's, and `cols` is ignored once the board has been measured (the column count is derived from the measured width at the parent's column pitch, so cells stay parent-sized as the container is resized) — `cols` only serves as the pre-measurement fallback. It never shrinks rows to fit, so pair it with an `isAutoHeight` container so the widget grows to fit its rows at that height.
  ...
  Widgets already on the board keep their cells. Narrowing the container lowers the column count, but a widget whose columns now fall outside it is **not** re-flowed into the narrower grid — it holds its position and is clipped by the container's edge, and widening the container brings it back exactly as it was. This is deliberate: resizing a container is not a request to rearrange the widgets inside it, and a layout that reshuffled on the way down could not be restored on the way back up.
...
- **`resizeGripPlacement`** `'inside' | 'corner' | 'outside'` — Override where this widget's grips sit, instead of letting it resolve from the widget's content.
...
- **Position:** `gridArea`, `order`, `gridColumn`, `gridRow`, `placeSelf`, `alignSelf`, `justifySelf`, `zIndex`, `blockMargin`, `inlineMargin`, `inset`, `blockInset`, `inlineInset`, `position`, `scrollMargin`, `blockScrollMargin`, `inlineScrollMargin`, `scrollPadding`, `blockScrollPadding`, `inlineScrollPadding`
...
Grip placement resolving itself across a nesting boundary, with no `resizeGripPlacement` set anywhere: the widget holding a `Board` draws its grips outside, in the gutter, and the leaf widgets keep theirs inside. Hover a child and only its grips appear; hover the container's outer edge and only the container's do.
