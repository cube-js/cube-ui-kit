...
block. Its members keep their exact offsets for the whole drag: the widgets it
climbs over move out of the way rather than being packed between them. When the
drag ends the board compacts normally, so a group whose members could each float
further settles on release. Under `compact="free"` (or `null`) nothing compacts,
