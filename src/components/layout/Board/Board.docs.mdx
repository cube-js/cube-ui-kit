...
Note that a widget clips its content only when it is a card. `isCard` means "this
widget draws a visible edge", and a borderless widget has no edge to clip to —
clipping one would crop any `outline` a descendant paints for its own focus or
active state. Set `widgetProps={{ overflow: 'hidden' }}` (or `overflow="hidden"`
per widget) to clip regardless.
...
### Card and borderless overflow

<Story of={BoardStories.BorderlessOverflow} />
