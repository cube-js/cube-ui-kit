...
- **`resizeGripPlacement`** `'inside' | 'corner' | 'outside'` — Where a widget's resize grips sit. Unset, **it resolves itself from the widget's content**: a widget holding a nested `Board` gets `'outside'`, and every other widget gets `'inside'`. Set it to override that, per board or per widget.
  ...
  - `'outside'` puts every grip in the grid gutter beyond the widget's edge, as a padded control of its own: a pill along each edge, a dot where two gutters cross. The control **is** the hit-zone — one element, so what you see is exactly what you can grab — and it never covers the widget's own content or a neighbour's.
  ...
  An `'outside'` grip needs gutter to sit in — at least 8px of `margin` on each axis. Below that it keeps its size and overhangs the neighbouring widgets, and the board warns in development; raise `margin`, or set `resizeGripPlacement` explicitly to put the grips back inside.
