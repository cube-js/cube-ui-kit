...
- **`column`** `number` (default: `0`) — Zero-based starting column inside a Grid or tab panel. A stack packs its children along its own axis and derives both coordinates, so it ignores whatever you store here.
- **`row`** `number` (default: `0`) — Zero-based starting row inside a Grid or tab panel. Derived inside a stack, as `column` is.
...
- `Space` selects a focused Dashboard node, and `Shift`, `Cmd`, or `Ctrl` with it extends the selection — the same modifiers a click takes.
...
- `Escape` during a pointer move, a pointer resize, or an add-region claim cancels the gesture and leaves the layout untouched. A cancelled gesture reports the arrangement it started from as its `commit` — the node through `info.items` and every sibling its previews had shuffled through `info.displaced` — so a consumer that applies previews must apply the commit too rather than treat it as a repeat.
