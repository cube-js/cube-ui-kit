...
- **`Dashboard.HorizontalStack`** — Children arranged across one row, sharing its full width in proportion to their `columns` and stretched vertically within their row constraints.
- **`Dashboard.VerticalStack`** — Children arranged vertically, sharing its full height in proportion to their `rows` and stretched horizontally within their column constraints.
...
A stack is the exception to fixed child spans. It always fills itself, sharing its own axis among its children in proportion to their stored spans and never below each child's own minimum, so resizing a stack stretches or squeezes what is inside it. A stack's own minimum is therefore the sum of its children's minimums rather than of their current spans, and its maximum the sum of their maximums. The stored span is a preference, not a position: it says how the space is shared, and Dashboard applies the distribution in layout rather than reporting it through `onPlacementChange`. While editing, a stack that still has room gains a narrow track of its own past the last child to hold its insertion point. Resizing a _child_ of a stack is a seam rather than a claim — it takes tracks from the next neighbour and gives them back the same way — so `onPlacementChange` reports the neighbour's new span in `info.displaced` and a size command is disabled when nothing can give way.
