...
By default the display element is keyboard-focusable (`tabIndex=0`, `role="button"`, `aria-roledescription="editable text"`). Pressing `Enter`, `F2` or `Space` enters edit mode without requiring a pointer. A keyboard focus ring (`#focus` token, rounded) is shown only when the element receives keyboard focus — clicks never trigger the ring.
