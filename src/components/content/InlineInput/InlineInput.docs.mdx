...
- **Form fields**: Use `TextInput` with a modern `<Form form={form}>` for inputs that need validation rules
...
- **`autoComplete`** `string` — HTML [`autocomplete`](https://developer.mozilla.org/en-US/docs/Web/HTML/Attributes/autocomplete) token forwarded to the inner input (`off` to suppress autofill on a rename field).
