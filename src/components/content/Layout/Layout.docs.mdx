...
- **`title`** `ReactNode` — Page/section title. A string title that runs out of room is truncated and shown in full in a tooltip; when the title and `extra` both overflow, they share the shortfall
...
<Story of={LayoutStories.StickyPanel} />
...
<Story of={LayoutStories.OverlayPanel} />
...
- [Layout Guide](/docs/getting-started-complex-layout--docs) - Patterns and examples for building layouts
- [Dialog](/docs/overlays-dialog--docs) - For modal panels
- [Item](/docs/content-item--docs) - Used by PanelHeader
