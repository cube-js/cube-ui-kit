...
| type | `string` | Applied based on type prop value (item, header, primary, secondary, outline, neutral, clear, link, alert) |
...
- `header` - Header appearance for section headers (only supports `default` theme, defaults to `descriptionPlacement="block"`)
...
- `special` - Special theme with unique styling (not available for `alert` or `header` types)
...
The `descriptionPlacement` prop controls where the description appears. It defaults to `"inline"` for most types, but `"block"` for `type="alert"` and `type="header"`:
