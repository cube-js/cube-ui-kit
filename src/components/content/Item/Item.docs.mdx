...
- **`children`** `string` — The content inside the Item
...
- **`prefix`** — Element rendered before the content (after icon)
- **`suffix`** — Element rendered after the content (before rightIcon)
- **`description`** `string` — Description text displayed with the item
- **`descriptionPlacement`** `'inline' | 'block'` (default: `inline`) — How the description is positioned relative to the main content
- **`size`** `'xsmall' | 'small' | 'medium' | 'large' | 'xlarge' | 'inline'` (default: `medium`) — Item size
- **`shape`** `'card' | 'button' | 'sharp' | 'pill'` (default: `button`) — Shape of the item border radius
