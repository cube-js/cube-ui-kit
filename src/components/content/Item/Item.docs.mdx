...
- `warning` - Yellow/amber theme for caution or attention (only available for `card` type)
- `note` - Violet theme for informational cards (only available for `card` type)
