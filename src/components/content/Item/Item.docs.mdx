...
| type | `string` | Applied based on type prop value (item, title, primary, secondary, outline, neutral, clear, link, alert) |
...
- `title` - Title appearance for section headers (only supports `default` theme, defaults to `descriptionPlacement="block"`)
...
- `alert` - Alert appearance for notifications (supports `default`, `success`, `danger`, `note` themes; defaults to `shape="card"`, `descriptionPlacement="block"`, and has `.5x` padding)
...
- `special` - Special theme with unique styling (not available for `alert` or `title` types)
- `note` - Note theme for informational alerts (only available for `alert` type)
...
- `card` - Card shape with larger border radius (`1cr`) — default for `type="alert"`
...
The `descriptionPlacement` prop controls where the description appears. It defaults to `"inline"` for most types, but `"block"` for `type="alert"` and `type="title"`:
...
Use `type="title"` for section headers. Title items have larger text preset and default to block description placement:

```jsx
<Item type="title" icon={<IconFolder />}>
  Section Title
</Item>

<Item type="title" icon={<IconFolder />} description="Section description">
...
Use `type="alert"` for notification-style items. Alert items automatically use card shape, block description placement, and have `.5x` padding. They support `default`, `success`, `danger`, and `note` themes:

```jsx
<Item type="alert" theme="default" icon={<IconInfo />} description="This is an informational message">
  Info Alert
</Item>

<Item type="alert" theme="success" icon={<IconCheck />} description="Operation completed successfully">
  Success Alert
</Item>

<Item type="alert" theme="danger" icon={<IconAlertTriangle />} description="Something went wrong">
  Error Alert
</Item>

<Item type="alert" theme="note" icon={<IconNote />} description="Additional context or tips">
  Note Alert
</Item>
```
