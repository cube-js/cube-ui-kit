...
- **`type`** `'item' | 'header' | 'primary' | 'outline' | 'clear' | 'link' | 'card'` (default: `item`) — Visual style variant
...
- **`type`** `string` — Applied based on type prop value (item, header, primary, outline, clear, link, card)
