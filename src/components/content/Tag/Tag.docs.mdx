...
- **`theme`** `'default' | 'primary' | 'danger' | 'success' | 'warning' | 'note' | 'special'` (default: `default`) — Visual theme of the tag
...
- **Primary**: Soft brand purple, for chips that reference a primary-colored entity
- **Special**: Solid brand purple, for highlighted or premium features
- **Disabled**: Gray, inactive or unavailable items

`primary` and `special` are the same brand hue at two weights. `primary` is the soft card — pale fill, tinted border, colored text — matching `note`, `success`, `danger` and `warning` in everything but hue, so reach for it when a chip should sit quietly beside its content. `special` is a solid fill with white text, which carries far more weight on the page.

```jsx
<Tag theme="primary">Claimed by Group A</Tag>
<Tag theme="special">Premium</Tag>
```
