...
The dual-backend shell (plan Phase 3) is in place: `Form/backend.ts` brands instances (`FORM_BACKEND`), `Form.tsx` exports the `<Form>` facade that renders `LegacyFormRoot` (the frozen engine) or the `ModernFormRoot` boundary (an error until the modern backend lands), and the root provides three contexts — the public legacy `FormContext`, `FormPresentationContext` and `LegacyFormBackendContext`. Radio/Checkbox groups mask all three through `FormScopeMask`. `useFieldProps` calls every hook in every mode; keep it that way. Shell behaviour is covered by `Form/dual-backend-shell.test.tsx` and the compile-time fixtures in `Form/form-types.test.tsx`.
