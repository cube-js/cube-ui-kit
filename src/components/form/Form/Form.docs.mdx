...
Use selectors for render-time state. `getValue`, `getValues`, `getActiveValues`, `getFieldSnapshot`, and `getSnapshot` are imperative reads; `subscribe` supports imperative observers. `values` contains retained data and `activeValues` contains registered fields. Commands include `setValue`, `setValues`, `batch`, `touch`, the defaults/reset commands, and explicit field/submit error setters. String paths are literal names; tuples such as `['rows', 0, 'amount']` address nested data. `setValue` accepts `{ source, touch, notify }`; programmatic changes do not touch or notify `onValuesChange` unless requested, while user changes do both by default. The creation-time `onValuesChange(values, change)` callback receives retained values and `{ names, source, kind }`.

The current modern root supplies presentation and controller context only. Named UI Kit inputs, `Form.Item`, `Form.Submit`, `Form.Reset`, `Form.SubmitError`, validation rules, and submission orchestration are not yet supported. Keep production input forms on `Form.useForm()` until those migration phases land. Modern roots reject `defaultValues` and root callback props; use controller creation options for defaults and `onValuesChange`. A root with a native `action` keeps browser submission; without an action, submission is prevented until the modern pipeline is available. Legacy wrappers such as `DialogForm` keep legacy-only types.
