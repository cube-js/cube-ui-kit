...
| 4 | Form identity change after mount | `instance-and-binding` | undefined | `<Form form={b}>` after mounting with `a` keeps using `a`. An input whose `form` prop changes registers with the new form **and stays registered in the old one** while mounted; since the dual-backend shell (Phase 3) unmounting releases the name from every form it was bound to (`../dual-backend-shell.test.tsx`; before, only the mount-time form released it). |
...
| 6 | Dynamic names | `instance-and-binding` | frozen | Renaming re-registers under the new name and drops the old value; a same-named sibling loses the shared field exactly as on unmount (row 5). Since the dual-backend shell (Phase 3) the renamed field is seeded like a first mount, so a field-level `defaultValue` becomes its baseline, and switching an input between named and standalone rebinds instead of throwing `Should have a queue…` (both pinned by `../dual-backend-shell.test.tsx`; the switch was undefined before). |
...
| 22 | Changes during validation | `validation` | frozen, undefined | `resetFieldsValidation()`, `resetFields()`, `setFieldValue()` and `setFieldsValue()` all discard the pending result; the caller's `validateField()` promise still settles. (A value change did **not** discard it before the stale-validation fix; that row was `bug-eligible` and flipped with the fix.) A result that lands after unmount still writes into the detached field object and rerenders the owner (undefined). Inline rule arrays do not trigger validation by themselves; rules are re-read at the next run. |
...
| `react-hooks/exhaustive-deps` | 22 | 2 | 20 |
