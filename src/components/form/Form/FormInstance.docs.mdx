...
This page describes `Form.useForm()` and `CubeFormInstance`. New modern forms use `Form.useController<Model>()`; see [Modern Form](/docs/forms-modernform--docs) for its commands and recipes. The methods and dot notation below are legacy-only.
