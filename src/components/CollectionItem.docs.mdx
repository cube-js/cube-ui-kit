...
|----------|----------|------|---------|
| `id` or `key` | ✅ **Required** | `string \| number` | Unique identifier - **critical for selection callbacks** |
| `textValue` | ⚠️ Recommended | `string` | For filtering & accessibility |
...
|----------|----------|--------|
| Static JSX | ✅ Required | Hardcoded value |
| Data-driven (`items`) | ✅ Required | Pass `item.key` or `item.id` |
| Using `.map()` | ✅ Required | Pass `item.key` or `item.id` |

| Component | Selection | Search |
|-----------|-----------|--------|
| ListBox | Single/Multiple | ❌ |
| FilterListBox | Single/Multiple | ✅ |
| Select | Single | ❌ |
| ComboBox | Single | ✅ |
| FilterPicker | Single/Multiple | ✅ |
| Picker | Single/Multiple | ❌ |
| Menu | Single action | ❌ |
| CommandMenu | Single action | ✅ |
