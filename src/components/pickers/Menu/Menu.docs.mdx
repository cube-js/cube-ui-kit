...
| keys | `string` | - | Keyboard shortcut (e.g., "Ctrl+C") |
...
```jsx
<Menu>
  <Menu.Item key="copy" icon={<CopyIcon />} keys="Ctrl+C">
    Copy
  </Menu.Item>
  <Menu.Item key="paste" icon={<PasteIcon />} keys="Ctrl+V">
    Paste
  </Menu.Item>
</Menu>
```
...
<Menu items={items}>
  {(item) => (
    <Menu.Item key={item.id} keys={item.shortcut}>
      {item.label}
    </Menu.Item>
  )}
</Menu>
...
3. **Keyboard Shortcuts**: Include shortcuts for common actions
   ```jsx
   <Menu.Item key="copy" keys="Ctrl+C">Copy</Menu.Item>
   ```
