...
- **`description`** `string` — Additional descriptive text for the group
...
- **`isDisabled`** `boolean` (default: `false`) — Whether the entire radio group is disabled
- **`isReadOnly`** `boolean` (default: `false`) — Whether the radio group can be focused but not changed
- **`isRequired`** `boolean` (default: `false`) — Whether selection is required before form submission
- **`isInvalid`** `boolean` (default: `false`) — Whether the radio group should display invalid visual styling
