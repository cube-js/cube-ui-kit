...
Supports the layout-related [Field properties](/docs/getting-started-field-properties--docs) (`label`, `description`, `message`, `labelPosition`, etc.). Note that SearchInput is not form-connected: it does not accept `name`/`rules` and does not participate in form state or validation.
...
SearchInput is not a form field. Use [TextInput](/docs/forms-textinput--docs) when the typed text has to be captured as a form value.
