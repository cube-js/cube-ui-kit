import { Meta, Story } from '@storybook/addon-docs/blocks';

import * as ColorPickerStories from './ColorPicker.stories.tsx';

<Meta of={ColorPickerStories} />

# ColorPicker

A swatch button that opens the color popover. It is the same popover [ColorInput](/docs/forms-colorinput--docs) uses — three perceptual axes backed by [Glaze](https://github.com/tenphi/glaze) — without the text field in front of it.

Reach for it when a color is **chosen but never typed**. Reach for `ColorInput` when someone needs to paste `#7a4dbf` from a brand guide.

## When to Use

- A toolbar or inspector where space is tight
- A chart legend, a tag color, a cell in a dense table
- Anywhere the exact notation is an implementation detail the user never sees

Use [ColorInput](/docs/forms-colorinput--docs) instead when the color has to be readable and pasteable as text.

## Component

<Story of={ColorPickerStories.WithValue} />

---

### Properties

- **`value`** `string | null` — The selected color (controlled)
- **`defaultValue`** `string | null` — The selected color (uncontrolled)
- **`onChange`** `(value: string | null) => void` — Fired with the color string
- **`format`** `'hex' | 'rgb' | 'hsl' | 'okhsl' | 'okhst' | 'oklch'` (default: `hex`) — Notation the value is written in, and the one shown on the trigger
- **`defaultSpace`** `'hst' | 'lch' | 'rgb'` (default: `hst`) — Color concept the popover opens with
- **`children`** `ReactNode` — Replaces the color shown on the trigger. Pass `null` for a swatch on its own
- **`placeholder`** `ReactNode` (default: `Pick a color`) — Shown on the trigger while there is no color
- **`type`** `CubeItemProps['type']` (default: `outline`) — Visual type of the trigger
- **`theme`** `CubeItemProps['theme']` (default: `default`) — Theme of the trigger
- **`size`** `CubeItemProps['size']` — Size of the trigger
- **`triggerTooltip`** `CubeItemProps['tooltip']` — Tooltip for the trigger, separate from `labelTooltip`
- **`swatches`** `Array<string | { color: string; label?: string }>` — Colors to offer under the editor, as a [ColorSwatchGroup](/docs/forms-colorswatchgroup--docs)
- **`swatchColumns`** `number` — How many swatches per row. Defaults to a single row
- **`isOpen`** `boolean` — Whether the popover is open (controlled)
- **`defaultOpen`** `boolean` (default: `false`) — Whether the popover is open initially
- **`onOpenChange`** `(isOpen: boolean) => void` — Fired when the popover opens or closes
- **`shouldFlip`** `boolean` (default: `true`) — Whether the popover may flip to the other side of the trigger
- **`placement`** `Placement` (default: `bottom start`) — Placement of the popover relative to the anchor, as a React Aria placement string (e.g. `'bottom start'`, `'top end'`)
- **`targetRef`** `RefObject<HTMLElement | null>` — The element the popover visually attaches to. Defaults to the trigger; use it to anchor the popover to an outer container so sibling controls' popovers line up

There is no `formatMode`: with no text to reconcile, `format` alone decides how the value is written. Everything else about the color model — the notations it reads, the gamut handling, the three spaces — is identical to `ColorInput` and documented there.

### Base Properties

Supports [Base properties](/docs/getting-started-base-properties--docs)

### Field Properties

Supports all [Field properties](/docs/getting-started-field-properties--docs)

### Styling Properties

#### styles

Customizes the wrapper around the trigger.

#### triggerStyles

Customizes the trigger button itself.

#### swatchStyles

Customizes the color swatch.

### Style Properties

These properties allow direct style application without using the `styles` prop:

- **Position:** `gridArea`, `order`, `gridColumn`, `gridRow`, `placeSelf`, `alignSelf`, `justifySelf`, `zIndex`, `margin`, `blockMargin`, `inlineMargin`, `inset`, `blockInset`, `inlineInset`, `position`, `scrollMargin`, `blockScrollMargin`, `inlineScrollMargin`, `scrollPadding`, `blockScrollPadding`, `inlineScrollPadding`
- **Dimension:** `width`, `height`, `blockSize`, `inlineSize`, `flexBasis`, `flexGrow`, `flexShrink`, `flex`
- **Block:** `border`, `blockBorder`, `inlineBorder`, `radius`, `shadow`, `outline`, `padding`, `blockPadding`, `inlinePadding`, `overflow`, `scrollbar`, `textAlign`

### Modifiers

The `mods` property accepts the following modifiers you can override:

| Modifier      | Type      | Description              |
| ------------- | --------- | ------------------------ |
| `placeholder` | `boolean` | No color is selected yet |

The swatch carries its own `empty` modifier while there is no color to show.

### The value as text

The popover carries a `ColorInput` above the preview, so the exact color can be read, copied, pasted and normalized in place rather than only dialed in. It has no trigger of its own — a disclosure inside the thing it discloses would be meaningless — and clearing it clears the picker.

### A palette under the editor

Pass `swatches` and a [ColorSwatchGroup](/docs/forms-colorswatchgroup--docs) appears below the channels, so a common color is one press away.

```jsx
<ColorPicker label="Brand color" swatches={palette} swatchColumns={4} />
```

That group never offers its own custom-color escape hatch, since the escape hatch is a `ColorPicker` and would nest popovers without end.

## Examples

### Basic usage

```jsx
<ColorPicker label="Series color" defaultValue="#7a4dbf" />
```

### A swatch on its own

Drop the label for a compact button — a toolbar, or a cell in a table.

<Story of={ColorPickerStories.SwatchOnly} />

```jsx
<ColorPicker aria-label="Series color" value={color} onChange={setColor}>
  {null}
</ColorPicker>
```

Give it an `aria-label` when you do: with no visible text, that is the only thing naming the control.

### A label of your own

```jsx
<ColorPicker value={color} onChange={setColor}>
  Accent
</ColorPicker>
```

### Trigger types

<Story of={ColorPickerStories.Types} />

### Inside a form

```jsx
function Example() {
  const form = Form.useController();
  return (
    <Form form={form} onSubmit={onSubmit}>
      <ColorPicker
        name="accent"
        label="Accent color"
        format="oklch"
        rules={[{ required: true, message: 'Pick an accent color' }]}
      />
    </Form>
  );
}
```

## Accessibility

### Keyboard Navigation

- `Tab` — moves focus to the trigger
- `Space` / `Enter` — opens the popover
- `Escape` — closes it
- `Left` / `Right`, `Home` / `End` on a channel slider — moves that channel
- `Left` / `Right` on the space switcher — moves between HST, LCH and RGB

### Screen Reader Support

- The trigger announces as a button, named by the field label, `aria-label`, or its own text
- The popover announces as a dialog named "Color picker"
- Each channel slider sits in a group named after its axis
- The swatch is decorative; the color is announced as the trigger's text unless you replace it

### ARIA Properties

- `aria-label` — names the trigger when there is no visible label or text
- `aria-expanded` — reflects the popover state

## Best Practices

1. **Do**: name a swatch-only picker

   ```jsx
   <ColorPicker aria-label="Series color">{null}</ColorPicker>
   ```

2. **Don't**: leave a swatch-only picker unnamed — the swatch says nothing to a screen reader

   ```jsx
   <ColorPicker>{null}</ColorPicker>
   ```

3. **Do**: reach for `ColorInput` the moment someone might want to paste a hex value; a picker alone makes that impossible.

## Integration with Forms

This component supports all [Field properties](/docs/getting-started-field-properties--docs) when used within a Form. The field value is the color string.

## Suggested Improvements

- A row of recent or preset swatches inside the popover
- An eyedropper that samples a pixel from the page, where [`EyeDropper`](https://developer.mozilla.org/en-US/docs/Web/API/EyeDropper) is available
- Clearing the color back to `null` from the trigger

## Related Components

- [ColorInput](/docs/forms-colorinput--docs) — the same popover behind a text field
- [Picker](/docs/forms-picker--docs) — the same trigger-plus-popover shape, for lists
- [HueSlider](/docs/forms-hueslider--docs) — just the hue axis
