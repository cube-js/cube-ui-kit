...
Customizes the root wrapper element of the ComboBox.
...
- **Async Loading**: Built-in support for async data loading with loading states per scroll position
- **Custom Option Rendering**: Allow more flexibility in rendering option content beyond text
- **Grouped Item Navigation**: Improve keyboard navigation to allow jumping between section groups
- **Empty State Customization**: Add more control over empty state rendering with custom components
