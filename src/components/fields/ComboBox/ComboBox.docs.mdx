...
- **`label`** `string` — Label text for the combo box
...
- **`isLoading`** `boolean` (default: `false`) — Whether the ComboBox is in a loading state
...
- **`isReadOnly`** `boolean` (default: `false`) — Whether the input can be selected but not changed
- **`isRequired`** `boolean` (default: `false`) — Whether user input is required before form submission
- **`validationState`** `'valid' | 'invalid'` — Whether the input should display valid or invalid visual styling
