...
Individual option items within the combo box dropdown. Each item is rendered using [ItemBase](/components/ItemBase) and supports all ItemBase properties for layout, icons, descriptions, and interactive features.
...
```jsx
<ComboBox label="Technologies" allowsCustomValue>
  <ComboBox.Item 
    key="react" 
    icon={<IconCode />}
...
| children | `ComboBox.Item[]` | - | Collection of ComboBox.Item components |
