...
- **`isDisabled`** `boolean` (default: `false`) — Whether the FilterListBox is disabled
- **`isLoading`** `boolean` (default: `false`) — Whether the FilterListBox is in loading state
- **`isRequired`** `boolean` (default: `false`) — Whether selection is required
