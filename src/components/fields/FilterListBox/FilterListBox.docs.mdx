...
- `searchable` `boolean` — Applied when the search input is rendered (hidden when `isLoadingItems={true}` and `allowsCustomValue={false}`)
...
Individual items within the FilterListBox. Each item is rendered using [Item](/docs/content-item--docs) and supports all Item properties.
