...
- **`emptyLabel`** `ReactNode` — Content shown when the collection is empty. When provided, the popover opens and displays this message; when omitted, the popover does not open for an empty collection
...
<Story of={PickerStories.CustomEmptyState} />

When `emptyLabel` is provided and the collection is empty, the popover opens and shows the message. Omit it to keep the popover closed.
