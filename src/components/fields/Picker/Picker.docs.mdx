...
Represents individual selectable items within the Picker dropdown. Each item is rendered using [Item](/docs/content-item--docs) and inherits all its properties.

#### Key Properties

See [Item documentation](/docs/content-item--docs) for the complete API. Common properties include:
...
An [ItemAction](/docs/actions-itemaction--docs) re-exported for the `actions` slot. It picks up the trigger's size and theme, so an action matches the field it sits in without being configured.
...
<Story of={PickerStories.WithActions} />
...
<Story of={PickerStories.DynamicSections} />
...
<Story of={PickerStories.Validation} />
...
This component supports all [Field properties](/docs/getting-started-field-properties--docs) when used within a Form context. The Picker automatically handles form validation, touched states, error messages, and integrates seamlessly with form submission.
