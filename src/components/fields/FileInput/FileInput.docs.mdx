...
- **`isDisabled`** `boolean` (default: `false`) — Whether the file input is disabled
- **`isReadOnly`** `boolean` (default: `false`) — Whether the file input can be focused but not changed
- **`isRequired`** `boolean` (default: `false`) — Whether file selection is required before form submission
- **`validationState`** `undefined | 'valid' | 'invalid'` — Whether the file input should display valid or invalid visual styling
