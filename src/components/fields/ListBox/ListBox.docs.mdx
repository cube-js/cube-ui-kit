...
- `DragHandle` - Grip handle icon shown when `isReorderable={true}`
...
Individual items within the ListBox. Each item is rendered using [Item](/docs/content-item--docs) and supports all Item properties for layout, icons, descriptions, and interactive features.

#### Item API

ListBox.Item is built using [Item](/docs/content-item--docs), which provides rich layout and interaction capabilities. All Item properties are supported:
...
<Story of={ListBoxStories.Reorderable} />
