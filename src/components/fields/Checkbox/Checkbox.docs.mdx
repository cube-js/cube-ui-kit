...
- **`label`** `string` — External label for the checkbox
...
- **`isReadOnly`** `boolean` (default: `false`) — Whether the checkbox can be focused but not changed
- **`isRequired`** `boolean` (default: `false`) — Whether the checkbox must be checked before form submission
- **`validationState`** `undefined | 'valid' | 'invalid'` — Whether the checkbox should display valid or invalid visual styling
