...
- **`tooltip`** `string | TooltipProviderProps` — Tooltip shown on hover over the box and its inline label, in or out of a group, including while disabled. A string, or `TooltipProvider` props such as `title` and `placement`. It opens on hover, not on keyboard focus, so keep anything essential in the label. The info badge next to a field label is `labelTooltip`
...
A disabled checkbox still shows its tooltip, which makes it a good place to say why the option is unavailable.

```jsx
<Checkbox isDisabled tooltip="Available on the Team plan">
  Single sign-on
</Checkbox>
```
