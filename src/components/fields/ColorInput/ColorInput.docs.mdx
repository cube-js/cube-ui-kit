...
- **`placement`** `Placement` (default: `bottom right`) — Placement of the popover relative to the anchor, as a React Aria placement string (e.g. `'bottom start'`, `'top end'`)
- **`targetRef`** `RefObject<HTMLElement | null>` — The element the popover visually attaches to. Defaults to the field wrapper; use it to anchor the popover to an outer container so sibling controls' popovers line up
...
This component supports all [Field properties](/docs/getting-started-field-properties--docs) when used within a Form. The field value is the color string, and clearing the input stores `null`, which makes `required` rules behave as expected.
