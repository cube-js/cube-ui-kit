...
- **`isDisabled`** `boolean` (default: `false`) — Whether the date picker is disabled
- **`isReadOnly`** `boolean` (default: `false`) — Whether the date picker can be focused but not changed
- **`isRequired`** `boolean` (default: `false`) — Whether date selection is required before form submission
- **`validationState`** `ValidationState` — Whether the date picker should display valid or invalid visual styling
