...
A grid of color swatches, one of which can be selected. It is the palette half of choosing a color, where [ColorPicker](/docs/forms-colorpicker--docs) is the freeform half — and the two compose: pass `swatches` to a `ColorPicker` and the palette appears under the editor.
