...
- **`autoFocus`** `boolean` (default: `false`) — Whether the search input should have autofocus
...
- **`isDisabled`** `boolean` (default: `false`) — Whether the picker is disabled
- **`isLoading`** `boolean` (default: `false`) — Whether the picker is in loading state
- **`isRequired`** `boolean` (default: `false`) — Whether the field is required
- **`validationState`** `'valid' | 'invalid'` — Validation state of the picker
