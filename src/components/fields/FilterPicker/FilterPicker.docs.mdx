...
- **`triggerTooltip`** `string | TooltipProps` — Tooltip for the trigger button (separate from `labelTooltip`)
...
- **`placement`** `Placement` (default: `bottom start`) — Placement of the popover relative to the anchor, as a React Aria placement string (e.g. `'bottom start'`, `'top end'`)
...
<Story of={FilterPickerStories.WithActions} />
...
<Story of={FilterPickerStories.Reorderable} />
