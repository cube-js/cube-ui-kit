...
- **`isDisabled`** `boolean` (default: `false`) — Whether the select is disabled
- **`isRequired`** `boolean` (default: `false`) — Whether user selection is required before form submission
- **`isLoading`** `boolean` (default: `false`) — Show loading spinner and disable interactions
- **`validationState`** `string` — Whether the select should display valid or invalid visual styling
...
**descriptionPlacement** -  how description is positioned: `inline`, `block`, or `auto`.
