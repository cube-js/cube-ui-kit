...
- **`clearLabel`** `string` — Accessible name for the built-in clear button. Defaults to a localized "Clear value".
...
- **`placement`** `Placement` (default: `${direction} end`) — Placement of the popover relative to the anchor, as a React Aria placement string (e.g. `'bottom start'`, `'top end'`)
- **`targetRef`** `RefObject<HTMLElement | null>` — The element the popover visually attaches to. Defaults to the trigger; use it to anchor the popover to an outer container so sibling controls' popovers line up
...
- **`tooltip`** `string | boolean | object` — Tooltip on the trigger (see [Notable Properties](#notable-properties)). The info badge next to the label is the field-level `labelTooltip`
...
**actions** - Your own controls inside the trigger, rendered to the left of the built-in clear button and dropdown caret. Pressing one runs its handler without opening or closing the popover — including while the popover is open — so an action can do something the clear button cannot, such as restoring a default selection instead of emptying it. Use `Select.Action` so they match the trigger's size and theme. Actions follow the trigger's disabled state but not `isReadOnly`; pass `isDisabled` yourself for an action that changes the value.
...
It belongs to the trigger alone. The info badge next to the field's label is `labelTooltip` — before that prop existed, a `tooltip` on a labelled Select rendered in both places.
...
Individual option items within the select dropdown. Each item is rendered using [Item](/docs/content-item--docs) and supports all Item properties for layout, icons, descriptions, and interactive features.
...
An [ItemAction](/docs/actions-itemaction--docs) re-exported for the `actions` slot. It picks up the trigger's size and theme, so an action matches the field it sits in without being configured. Give every icon-only action an `aria-label` (a `tooltip` string also supplies one), since the icon alone has no accessible name.

```jsx
<Select
  label="Color"
  actions={<Select.Action icon={<ReloadIcon />} aria-label="Reset" onPress={reset} />}
>
  {items}
</Select>
```
...
<Story of={SelectStories.WithActions} />
...
<Story of={SelectStories.Validation} />
...
This component supports all [Field properties](/docs/getting-started-field-properties--docs) when used within a Form. It integrates seamlessly with the form validation system and supports both controlled and uncontrolled modes.
