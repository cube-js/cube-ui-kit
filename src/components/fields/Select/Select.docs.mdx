...
**actions** - Your own controls inside the trigger, rendered to the left of the built-in clear button and dropdown caret. Pressing one runs its handler without opening the popover, so an action can do something the clear button cannot — such as restoring a default selection instead of emptying it. Use `Select.Action` so they match the trigger's size and theme.
