...
- **`emptyLabel`** `ReactNode` — Label shown when the option list is empty. When provided, the popover stays open and displays this message; when omitted, the popover does not open for an empty collection
...
<Story of={SelectStories.CustomEmptyState} />

When `emptyLabel` is provided and there are no options, the popover opens and shows the message. Omit it to keep the popover closed.
