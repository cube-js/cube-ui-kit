...
Form-aware inputs continue to use the existing `useFieldProps` integration. The modern Form migration currently exposes controller creation and selectors for state consumers; modern field binding is a later phase. Do not register a modern field during render or connect a modern controller through the legacy input path. See [Form](/docs/forms-form--docs) for the current integration boundary.
