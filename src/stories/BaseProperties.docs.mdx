...
<Meta title="Getting Started/Base Properties" />
...
- **`isChecked`** `boolean` — Renders `data-checked`, which the `checked` modifier resolves against in `styles`
...
- [CreateComponent](/docs/getting-started-create-component--docs) — Building components with base properties
