...
const Card = tasty({
  styles: {
    ...
    border: '1bw solid #accent',   // → 1px solid var(--purple-color)
  },
});
...
Recipes are predefined, named style bundles that can be applied to any component via the `recipe` style property. Think of them like CSS utility classes, but without cascade issues -- component styles always override recipe values.
...
const Card = tasty({
  styles: {
    recipe: 'card',
    color: '#text', // Component styles override recipe values
  },
});

// Compose multiple recipes (merged in order, space-separated)
...
**Merging order:** `base_recipe_1 base_recipe_2 → component styles → post_recipe_1 post_recipe_2`. All merges use `mergeStyles` semantics: primitives and state maps with a `''` key fully replace; state maps without `''` extend the existing value.

**Post-merge recipes (`|` separator):**

Recipes listed after `|` are applied *after* component styles using `mergeStyles`, allowing recipe state maps to extend component values instead of being overridden by them:
...
const Input = tasty({
  styles: {
    ...
    preset: 't3',
    //       ^^^^^^^^^^   ^^^^^^^^^^^^^^^
    //       base group    post group
  },
});
// Result: preset becomes { '': 't3', ':-webkit-autofill': 'inherit' }
// The post recipe's state map extends the component's primitive value.
```

**Key rules:**
- Recipe names are space-separated within each group
- `|` separates base (before component styles) from post (after component styles) groups
- `|` is optional — if absent, all recipes are base
- Recipes are flat styles — they cannot contain sub-element keys
- Recipes cannot reference other recipes — for composition, list multiple recipes: `recipe: 'card elevated rounded'`
- Use `recipe: ''` (empty string) to clear recipes when extending a base component
- Recipes may contain `@keyframes`, `@properties`, and token definitions (`$name`/`#name`)

**Performance:** When no `recipe` key is present in styles, the overhead is negligible (a single property check). When recipes are used, styles are resolved once and then enter the normal caching pipeline.
...
// Usage
padding: '2sp';  // → 16px (calculated: 2 * 8px)
padding: '1sp';  // → 8px
padding: '.5sp'; // → 4px
```
...
configure({
  units: {
    x: '8px',  // Base unit
    y: '2x',   // References x → 2 * 8px = 16px
    z: '2y',   // References y → 2 * 16px = 32px
  },
});

// Usage
padding: '1x'; // → 8px
padding: '1y'; // → 16px (resolved from 2x)
padding: '1z'; // → 32px (resolved from 2y → 4x)
padding: '3y'; // → 48px (3 * 16px)
```
...
- Function only: `fill: (props) => ...` — lookup style inferred from key name
- Single tuple: `['styleName', handler]` — explicit lookup style
- Multi tuple: `[['style1', 'style2'], handler]` — multiple lookup styles
...
A predefined, named style bundle registered via `configure({ recipes })`. Applied to components via the `recipe` style property. Multiple recipes are space-separated. Base recipes (before `|`) are merged first, then component styles, then post recipes (after `|`). All merges use `mergeStyles` semantics.

```jsx
// In config
recipes: { card: { padding: '4x', fill: '#surface', radius: '1r' } }

// In styles — base recipes (component styles override)
styles: { recipe: 'card', color: '#text' }

// Post recipes (extend component styles via mergeStyles)
styles: { recipe: 'reset input | input-autofill', preset: 't3' }
```
...
// → style="--spacing: calc(2 * var(--gap)); --primary-color: var(--purple-color); --primary-color-rgb: var(--purple-color-rgb)"
```
...
color: '#purple.$disabled', // Dynamic opacity via CSS custom property

// #current - inherits from CSS currentcolor
fill: '#current',           // → currentcolor
fill: '#current.5',         // → color-mix(in oklab, currentcolor 50%, transparent)
fill: '#current.$fade',     // → color-mix with dynamic opacity
...
> **Note:** `#current` is a reserved token that cannot be overridden via `configure({ tokens: {...} })`.
...
fill: '#white #dark.1',           // Dual-fill: base color + overlay (for transitions)
...
// Dual-fill: Two colors create a layered background
// First color → background-color, second → background-image gradient
// Enables CSS transitions on semi-transparent overlays
fill: '#white #purple.1',         // White base with purple overlay
```
...
```jsx
// Boolean mods
mods={{ hovered: true }}         // → data-hovered=""
mods={{ isSelected: true }}      // → data-selected="" (preserves 'is' via kebab-case)

// String value mods
mods={{ theme: 'danger' }}       // → data-theme="danger"
mods={{ size: 'large' }}         // → data-size="large"
mods={{ variant: 'primary' }}    // → data-variant="primary"

// Number mods (converted to string)
mods={{ level: 2 }}              // → data-level="2"

// Empty string (explicit empty value)
mods={{ label: '' }}             // → data-label=""
...
// → data-hovered="" data-theme="danger" data-size="large"
```
...
State keys (for both regular modifiers and advanced `@...` states) support boolean logic with a predictable precedence (highest → lowest):
...
> **💡 Class Selector Note:** When using `.class` in state keys, it's appended directly to the element selector without a space, targeting the **same element** (e.g., `.tasty-class.active`). This is different from sub-element keys at the style root, which create descendant selectors.
...
If you repeat the same advanced state in many places, define an alias once and reuse it. You can define aliases globally via `configure({ states })`, locally per component as top-level `@name` keys in the styles object, or scoped to a specific sub-element by defining `@name` inside a sub-element block (see [Scoped Predefined States](#scoped-predefined-states)).
...
styles: {
  gridColumns: {
    ...
    '@media(w <= 1200px)': '1fr 1fr', // ≤1200px: 2 columns
    '@media(w <= 768px)': '1fr',      // ≤768px: 1 column
  },
  padding: {
    '': '4x',
...
**Style query shorthand inside `@(...)`:**

- `$name` → checks for existence of `--name` (`style(--name)`)
- `$name=value` → checks equality (`style(--name: "value")`)
...
**CSS function syntax inside `@(...)`:**

Any CSS function-like syntax (e.g., `scroll-state()`, `style()`) is passed through to the `@container` rule verbatim. This means current and future container query functions work out of the box.

```jsx
styles: {
  position: {
    '': 'relative',
    '@(scroll-state(stuck: top))': 'sticky',                  // @container scroll-state(stuck: top)
    '@(nav, scroll-state(stuck: top))': 'sticky',             // @container nav scroll-state(stuck: top)
  },
  color: {
    '': '#text',
    '@(style(--theme: dark))': '#white',                      // @container style(--theme: dark)
  },
}
```

Note: The `$` shorthand (`$name`, `$name=value`) is still the recommended way to query CSS custom properties since it automatically adds the `--` prefix and wraps the value in quotes. Use the raw `style()` function when you need to query regular CSS properties or need full control over the syntax.
...
- `key=value` → `:root[data-key="value"]` (kebab-cased)
- `camelCaseFlag` → `:root[data-camel-case-flag]`
- `.className` → `:root.className`
- `[attr]` / `[attr="value"]` → `:root[...]`

#### Own States (`@own`)

Apply a sub-element’s **own** state (not the parent’s). This only makes sense inside a sub-element style block (Tasty will warn if you use `@own(...)` at the root level, where it’s equivalent to the inner condition).
...
> **⚠️ Important:** Place more specific rules last - they take precedence.

#### Composing States

Each state in a state key is a **single token**. You cannot concatenate two states into one token — use the `&` operator to combine them:

```jsx
styles: {
  '@autofill': ':-webkit-autofill',

  // ❌ Wrong — @autofill:hover is two states written as one token
  color: { '@autofill:hover': '#blue' },

  // ✅ Correct — use & to compose separate states
  color: { '(@autofill & :hover)': '#blue' },

  // ✅ Multiple variants with |
  fill: {
    '@autofill | (@autofill & :hover) | (@autofill & :focus)': '#white',
  },
}
```
...
#### Scoped Predefined States

Predefined states (`@name`) can be defined inside sub-element blocks. They are **scoped** to that sub-element and any deeper nesting — they do not leak to sibling sub-elements or the parent:

```jsx
styles: {
  '@shared': ':hover',  // available everywhere

  Label: {
    '@active': ':focus',                          // scoped to Label only
    color: { '': '#text', '@active': '#primary' },

    '&::placeholder': {
      color: { '@active': 'transparent' },        // inherited from Label
    },
  },

  Icon: {
    // ❌ @active is NOT available here — it's scoped to Label
    // ✅ @shared IS available — defined at the parent level
    color: { '': '#text', '@shared': '#primary' },
  },
}
```

A sub-element `@name` overrides a same-named parent `@name` within that scope. Parent-level states are always inherited.

#### Selector Affix (`$`)

Control sub-element selector using the `$` property. The key `[data-element="Key"]` is auto-injected based on pattern:
...
| `>Body>Row` | `> [Body] > [Row] [el]` | Descendant of chain |
| `::before` | `::before` | Root pseudo (no key) |
| `@:hover` | `[el]:hover` | Key with pseudo |
| `>@.active` | `> [el].active` | Key with class |
| `@[disabled]` | `[el][disabled]` | Key with attribute |
| `>Item+` | `> [Item] + [el]` | After sibling |
| `::before, ::after` | Both pseudos | Multiple selectors |
| `>ul>li` | `> ul > li [el]` | HTML tags |
| `a:hover` | `a:hover` | Tag with pseudo (no key) |

Use `@` placeholder to control key position. Without `@`, key is auto-injected at the end unless pattern ends with pseudo/class/attribute.

```jsx live=false
const Component = tasty({
  styles: {
    Row: { $: '>' },              // Direct child
    Cell: { $: '>Body>Row>' },    // Chained path
    Before: { $: '::before' },    // Root pseudo-element  
    Item: { $: '>@:hover' },      // Key with pseudo
  },
});
```
...
const List = tasty({
  styles: {
    border: {
      '': 'none',
      ':has(Body>Row)': '1bw solid #border',    // Compact syntax supported
    },
...
Define CSS `@property` at-rules directly in styles using tasty token syntax. This enables advanced features like animating custom properties with proper type checking and initial values.

#### Token Syntax

Properties use the same token syntax as other tasty values:

| Token | CSS Output | Description |
|-------|-----------|-------------|
| `$name` | `--name` | Regular property |
| `#name` | `--name-color` | Color property (auto-sets `syntax: '<color>'`) |

For **color tokens** (`#name`):
- `syntax` is automatically set to `'<color>'` (cannot be overridden)
...
const AnimatedGradient = tasty({
  styles: {
    '@properties': {
      ...
      '#theme': {
        initialValue: 'purple', // syntax: '<color>' is auto-set
      },
    },
...
**Key features:**
- Properties are registered once and are permanent (never removed from the document)
- If the same property is registered with different options, a warning is emitted
- Color tokens automatically get `syntax: '<color>'` and default to `transparent`

**Property options:**

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `syntax` | `string` | `'*'` (or `'<color>'` for `#name`) | CSS syntax (e.g., `'<angle>'`, `'<length>'`, `'<number>'`) |
| `inherits` | `boolean` | `true` | Whether the property inherits from parent elements |
| `initialValue` | `string \| number` | - (`'transparent'` for `#name`) | Initial value for the property |

**Common syntax values:**

| Syntax | Description | Example values |
|--------|-------------|----------------|
| `<color>` | Any color value | `red`, `#ff0000`, `rgb(255,0,0)` |
| `<length>` | Length values | `10px`, `2em`, `50%` |
| `<angle>` | Angle values | `45deg`, `0.5turn`, `3.14rad` |
| `<number>` | Numeric values | `0`, `1.5`, `-10` |
| `<percentage>` | Percentage values | `50%`, `100%` |
| `<length-percentage>` | Length or percentage | `10px`, `50%` |
| `*` | Any value (default) | Any CSS value |

#### Animating Custom Properties

One of the main use cases for `@property` is enabling CSS transitions on custom properties:

```jsx
const AnimatedCard = tasty({
  styles: {
    '@properties': {
      '$card-scale': {
        syntax: '<number>',
    ...
    transition: '$$card-scale 0.2s ease-out',
    '$card-scale': {
      '': 1,
      hovered: 1.05,
    },
...
For imperative property registration, use the `useProperty` hook:

```jsx
import { useProperty } from '@cube-dev/ui-kit';

function AnimatedComponent() {
  // Register a property using token syntax
  ...
  // Color properties with token syntax (auto-sets syntax: '<color>')
  useProperty('#accent', {
    initialValue: 'purple', // defaults to 'transparent' if not specified
  });

  // Legacy format (--name) is also supported
  useProperty('--scale-factor', {
    syntax: '<number>',
    initialValue: 1,
  });
...
The hook accepts the same token syntax as `@properties` in styles:
- `$name` → registers `--name`
- `#name` → registers `--name-color` with auto-set `syntax: '<color>'`
- `--name` → legacy format, still supported

#### Global Properties via configure()

Define properties globally that are available to all components. Properties are registered once when the first style is injected:
...
configure({
  properties: {
    '$rotation': { syntax: '<angle>', inherits: false, initialValue: '0deg' },
    '$scale': { syntax: '<number>', initialValue: 1 },
    '#accent': { initialValue: 'purple' }, // syntax: '<color>' auto-set
  },
});

// Now any component can use these properties without defining them locally
const Spinner = tasty({
  styles: {
...
**Key features:**
- Global properties are injected once when styles are first generated
- Same token syntax as local properties (`$name`, `#name`)
- Must be configured before any component renders
...
When you need to reference the literal CSS property name (not its value), use the double-prefix syntax:

| Syntax | Output | Use case |
|--------|--------|----------|
| `$$name` | `--name` | Reference the property name itself |
| `##name` | `--name-color` | Reference the color property name itself |

This is useful in CSS properties like `transition`, `animation`, or `will-change` where you need the property name:

```jsx
const Component = tasty({
  styles: {
    // $name → var(--name) (the value)
    // $$name → --name (the property name)
    transition: '$$rotation 0.2s ease-out, ##accent 0.3s',
    // Outputs: transition: --rotation 0.2s ease-out, --accent-color 0.3s

    // Other use cases
    'will-change': '$$scale, ##theme',
    // Outputs: will-change: --scale, --theme-color
  },
});
```

> **Important:** The `##name` syntax automatically adds the `-color` suffix. Use `##theme` to get `--theme-color`, **not** `##theme-color` (which would incorrectly produce `--theme-color-color`). This mirrors how `#name` in `@properties` defines `--name-color`.
...
const Component = tasty({
  styles: {
    // ✅ Correct: tasty syntax with parentheses
    padding: '(1 / 10 * $size)',
    width: '($base-width + 2x)',
    
    // ❌ Avoid: raw CSS calc() - use tasty syntax instead
...
// ✅ Define custom color tokens with #name syntax
styles: {
...
The `useRawCSS` hook injects **plain CSS text** (without tasty parsing). It’s useful for global resets, importing third‑party snippets, or when you already have authored CSS and just need lifecycle-managed injection/cleanup.
...
The `useMergeStyles` hook merges a base `styles` object with separate sub-element style props. This simplifies the common pattern of accepting props like `tabListStyles`, `prefixStyles`, etc. that should merge into `styles.TabList`, `styles.Prefix`, etc.

```tsx
import { useMergeStyles } from '@cube-dev/ui-kit';

function MyTabs({ styles, tabListStyles, prefixStyles, suffixStyles, ...rest }) {
...
**When to use `useMergeStyles`:**
- Creating components that accept separate style props for sub-elements (e.g., `triggerStyles`, `popoverStyles`)
- Simplifying the merging of user-provided sub-element styles with base styles
- Maintaining a clean API where users can style sub-elements via dedicated props

**Parameters:**

| Parameter | Type | Description |
|-----------|------|-------------|
| `styles` | `Styles \| undefined` | Base styles object |
| `subStylesMap` | `Record<string, Styles \| undefined>` | Mapping of sub-element names to their additional styles |

**Returns:** `Styles` - Merged styles object ready to pass to a tasty component.

**Features:**
- Memoized to avoid unnecessary object creation on re-renders
- Leverages `mergeStyles` internally for proper deep merging of sub-element styles
- Handles `undefined` values gracefully (they are skipped)

```tsx
// Example: Creating a Picker component with separate style props
...
function MyPicker({ styles, triggerStyles, popoverStyles, listBoxStyles, ...rest }: MyPickerProps) {
  const mergedStyles = useMergeStyles(styles, {
  ...
  return <PickerElement styles={mergedStyles} {...rest} />;
}

// Usage
<MyPicker
  styles={{ padding: '2x' }}
  triggerStyles={{ fill: '#surface' }}
  popoverStyles={{ shadow: '2x 2x 4x #shadow' }}
/>
```
...
- **Hooks**: See [Hooks](#-hooks) for `useStyles`, `useGlobalStyles`, `useRawCSS`, and `useMergeStyles`
