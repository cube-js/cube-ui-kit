...
Tasty supports advanced CSS at-rules and contextual states using `@` prefix:
...
styles: {
  ...
  fill: {
    '': '#surface',
    '@(sidebar, $variant=danger)': '#red.05',  // Style query on named container
  },
}
```
...
Apply sub-element's own state (not parent's):
...
> **💡 Tips:**
> - Use `&` to combine states, `|` for alternatives, `!` for negation
> - Advanced states generate exclusive CSS (non-overlapping selectors)
> - Place more specific rules last (higher priority)
