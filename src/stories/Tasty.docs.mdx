...
Control sub-element selector using the `$` property. The system auto-injects `[data-element="Key"]` based on the pattern:

```jsx
const Table = tasty({
  styles: {
    // Direct child: trailing > injects key
    Row: {
      $: '>',                     // → .table > [data-element="Row"]
      padding: '1x',
    },
    
    // Chained selectors (compact syntax supported)
    Cell: {
      $: '>Body>Row>',            // → > [data-element="Body"] > [data-element="Row"] > [data-element="Cell"]
      border: '1bw solid #border',
    },
    
    // Descendant of element (no trailing >)
    Text: {
      $: '>Body>Row',             // → > [data-element="Body"] > [data-element="Row"] [data-element="Text"]
      color: '#text',
    },
    
    // Default: descendant selector (no $)
    Label: {
      color: '#text',             // → .table [data-element="Label"]
    },
  },
});
```

**Pseudo-elements on root** — no key injection:
...
**Pseudo on sub-element** — use `@` placeholder:

```jsx
const List = tasty({
  styles: {
    Item: {
      $: '>@:hover',              // → > [data-element="Item"]:hover
      fill: '#hover',
    },
    Label: {
      $: '@::before',             // → [data-element="Label"]::before
      content: '"•"',
    },
  },
});
...
**Sibling combinators** — must follow an element:

```jsx
const List = tasty({
  styles: {
    Next: {
      $: '>Item+',                // → > [data-element="Item"] + [data-element="Next"]
      marginTop: '1x',
    },
    // ❌ Invalid: standalone + or ~ (targets outside root)
    // Bad: { $: '+' }
  },
});
```
...
const Container = tasty({
  styles: {
    Deco: {
      $: '::before, ::after',     // Applies to both pseudo-elements
      content: '""',
    },
  },
});
...
**HTML tag selectors** — lowercase names target native elements:

```jsx
const Nav = tasty({
  styles: {
    // Style all <a> tags inside, key injected as descendant
    Link: {
      $: 'a',                     // → .nav a [data-element="Link"]
      textDecoration: 'none',
    },
    // Direct child tags
    Item: {
      $: '>li',                   // → > li [data-element="Item"]
      padding: '1x',
    },
    // Chained tags
    ListItem: {
      $: '>ul>li',                // → > ul > li [data-element="ListItem"]
      margin: '0',
    },
    // Tag with pseudo (no key injection)
    HoverLink: {
      $: 'a:hover',               // → a:hover (no key)
      color: '#primary',
    },
    // Mixed with elements
    Text: {
      $: '>Body>span',            // → > [data-element="Body"] > span [data-element="Text"]
      color: '#text',
    },
  },
});
...
**Key injection rules:**
1. `@` present → inject key at `@` position
2. Ends with combinator (`>`, `+`, `~`) → inject key after it
3. Ends with uppercase element → inject key as descendant
4. Ends with lowercase tag → inject key as descendant
5. Otherwise (pseudo, class, attribute) → no injection
