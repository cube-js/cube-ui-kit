...
import { Form, Field, Submit, Input, DialogForm } from '../index';
...
```tsx
import { Form, Field, Input } from '@cube-dev/ui-kit';
...
<Canvas>
  <Story name="LoginForm">
    {(args) => (
      <Form
        onSubmit={async (...submitArgs) => {
          await wait(1000);
          action('onSubmit')(...submitArgs);
        }}
        requiredMark={false}
        {...args}
      >
        ...
        >
          <Input.Text type="email" label="Email" autoComplete="email" />
        </Field>
        <Field
          name="password"
          rules={[
