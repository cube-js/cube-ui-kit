...
For modern forms, follow the [use-case guide](/docs/forms-modernform--docs). Put binding and validation options on `form.field(path, options)` and presentation props on the input. The prop reference below also covers `name` bindings for shared controls and legacy forms; these are alternatives to the typed descriptor, not additional required setup.
...
- **`name`** `string` — Field name for form data binding when a descriptor is not supplied.
