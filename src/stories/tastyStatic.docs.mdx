...
Recipes work with `tastyStatic` the same way as with runtime `tasty`. Define recipes in the plugin config and reference them via the `recipe` style property:
...
Recipes are resolved at build time before CSS extraction, so they have zero runtime overhead.
